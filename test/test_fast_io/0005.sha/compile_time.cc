...
constexpr auto evaluate_sha512() noexcept {
    ...
    std::array<char8_t,
               print_reserve_size(fast_io::io_reserve_type<
                                  char8_t, decltype(hash_digest(
                                               fast_io::sha512_context {}))>)>
        array {};
    fast_io::sha512_context ctx;
    ctx.do_final();
    print_reserve_define(
        fast_io::io_reserve_type<char8_t, decltype(hash_digest(
                                              fast_io::sha512_context {}))>,
...
//	std::array<char8_t,print_reserve_size(fast_io::io_reserve_type<char8_t,T>)>
//array{}; 	T sha("key"); 	fast_io::hash_processor processor{sha};
//	print_freestanding(processor,"The quick brown fox jumps over the lazy
//dog"); 	processor.do_final();

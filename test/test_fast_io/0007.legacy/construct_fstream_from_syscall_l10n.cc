...
int main() {
/*
This is an example to explain how fast_io's files work with each other, and how
you could use fast_io with existing FILE* or fstream apis
*/
#if (defined(_WIN32) && !defined(__WINE__)) || defined(__CYGWIN__)
#ifdef _WIN32_WINDOWS
    //_WIN32_WINDOWS macro targets WIN9X kernel (windows 95, windows 98 and
    //windows ME) On 9x kernel, although ntdll.dll does exist, it does not do
    // anything.
    ...
    *fout.rdbuf() = std::move(*fbf.fb);
    ...
    fast_io::posix_tzset();
    auto unix_ts { fast_io::posix_clock_gettime(
        fast_io::posix_clock_id::realtime) };
    fast_io::native_l10n l10n(u8"");
    println(
        imbue(l10n, fast_io::c_stdout()), "Unix Timestamp:", unix_ts,
        "\n"
        "Universe Timestamp:",
        ...
        "\n"
        "win32 HANDLE:",
        ::fast_io::mnp::handlevw(
            static_cast<fast_io::win32_io_observer>(fiob).handle)
        ...
        "zw HANDLE:",
        ::fast_io::mnp::handlevw(
            static_cast<fast_io::zw_io_observer>(fiob).handle),
        "\n"
        "nt HANDLE:",
        ::fast_io::mnp::handlevw(
            static_cast<fast_io::nt_io_observer>(fiob).handle)
#endif
#endif
    );
}
...
g++ -o construct_fstream_from_syscall construct_fstream_from_syscall.cc -Ofast
-std=c++20 -s -flto -march=native -lntdll
...
D:\hg\fast_io\examples\0007.legacy>g++ -o construct_fstream_from_syscall_l10n
construct_fstream_from_syscall_l10n.cc -Ofast -std=c++23 -s -flto -march=native
-I../../include -lntdll

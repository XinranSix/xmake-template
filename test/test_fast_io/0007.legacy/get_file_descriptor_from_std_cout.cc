...
/*
C++ iostream is a holy crap. You cannot access FILE* inside std::cout because of
the OO garbage. See an old blog: https://www.ginac.de/~kreckel/fileno/

Also see wiki in fast_io to understand how C++ std::fstream work. It is
implemented by FILE*. So why do you use iostream?? It is just a garbage quality
of RAII wrapper for FILE*.
...
int main() {
    ...
    //  I hacked ALL c++ standard library implementation, including libstdc++,
    //  libc++ and MSVC stl
    ...
    println("std::cout.rdbuf():", handlevw(siob.fb),
            ...
            ,
            "\nwin32 HANDLE:",
            handlevw(static_cast<fast_io::win32_io_observer>(siob).handle),
            "\n"
...
// Zw and Nt are the same in user mode, but different in kernel mode. See Book:
// Windows Internal 7th version Or MSDN What Does the Zw Prefix Mean?

...
int main(int argc, char const **argv) {
    ...
    fast_io::u8iobuf_socket_file socket { fast_io::tcp_connect(fast_io::to_ip(
        fast_io::native_dns_file(::fast_io::mnp::os_c_str(argv[1])), 80)) };
    print(
        socket,
        u8"GET / HTTP/1.1\r\n"
        "Host:",
        ::fast_io::mnp::code_cvt_os_c_str(
            argv[1]), // this may be dangerous but here we just show the example
        u8"\r\n"
        "User-agent:whatever\r\n"
        "Accept-Type:*/*\r\n"
        "Connection:close\r\n\r\n");
